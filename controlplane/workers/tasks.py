...
# A tenant's own repository has no shared build contract the way this
# platform's internal app/ monorepo does (one generic Dockerfile, one
# fixed layout) — so when a repo has no Dockerfile, the only thing worth
# trying is the single most common real case: a plain FastAPI app with a
# requirements.txt and an `app = FastAPI(...)`-style entrypoint. Anything
# else still fails closed with the original message; this is a narrow,
# best-effort convenience, not a general buildpack.
_AUTODETECT_ENTRYPOINTS = ("main.py", "app.py")
_FASTAPI_APP_RE = re.compile(r"\bapp\s*=\s*(?:fastapi\.)?FastAPI\s*\(")

_AUTO_DOCKERFILE_TEMPLATE = """\
# Auto-generated by the platform: no Dockerfile was found in this repository,
# but {entrypoint}.py looks like a FastAPI app (a requirements.txt alongside
...
# pre-deploy Trivy gate blocks on HIGH, so without this every auto-built
# image would be blocked and the whole no-Dockerfile path would be useless.
# Same treatment app/Dockerfile applies to this platform's own services.
RUN apt-get update \\
    && apt-get upgrade -y \\
...
def _detect_fastapi_entrypoint(repo_dir: Path) -> str | None:
    """Return the module name (no .py) of a detected FastAPI entrypoint, or
    None if this doesn't look like a plain FastAPI app we can safely build."""
    if not (repo_dir / "requirements.txt").is_file():
        return None
    for filename in _AUTODETECT_ENTRYPOINTS:
        ...
        try:
            ...
            continue
        if "fastapi" in text.lower() and _FASTAPI_APP_RE.search(text):
            return filename[: -len(".py")]
    return None


def _try_autogenerate_dockerfile(repo_dir: Path, port: int) -> str | None:
    """Write a Dockerfile for a detected FastAPI app into repo_dir. Returns
    the entrypoint module used, or None if nothing was detected (caller
    should fail closed with the original "no Dockerfile" error)."""
    entrypoint = _detect_fastapi_entrypoint(repo_dir)
...
def _render_manifests(project: Project, deployment: Deployment, image_ref: str) -> list[Path]:
    ...
    context = {
        ...
        "secrets": secrets,
        # Passed through as-is: empty means "TCP probe the port", which is
        # what a tenant app that does not serve this platform's own /livez
        # needs. An `or "/livez"` fallback here would silently override that
        # and CrashLoop the very apps the empty default exists for.
