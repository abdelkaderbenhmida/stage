...
`products-service` — a demo FastAPI service exposing a product-listing endpoint, deployed
through the control plane's pipeline. On startup it loads `DATABASE_URL` and `API_KEY` from
Vault via `shared.vault_client`; in `dev`/`development`/`local` environments a missing secret
falls back to an in-memory SQLite URL and a per-process ephemeral token (logged as a
...
- `main.py` — FastAPI app with structured logging and Prometheus instrumentation
  ...
  (Vault-health-gated), and `GET /products` (returns a static list of `{id, name, price}`).
- `requirements.txt` — service-specific dependencies (`fastapi`, `uvicorn`,
  `prometheus-fastapi-instrumentator`, `hvac`).
- `service.yaml` — ArgoCD ApplicationSet discovery marker (`name: products-service`,
  image `tag`).
