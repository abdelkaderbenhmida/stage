...
_LOG = setup_logging("zz-e2e")

app = FastAPI(title="zz-e2e", version="1.0.0")
...
SERVICE_NAME = os.environ.get("SERVICE_NAME", "zz-e2e")
VAULT_CONFIGURED = bool(os.environ.get("VAULT_ADDR"))


@app.get("/")
def root():
    return {"service": "zz-e2e", "version": "1.0.0", "vault_configured": VAULT_CONFIGURED}
...
def readyz():
    ...
    return JSONResponse(
        ...
        content={"service": "zz-e2e", "vault": health},
    )
